...
def L3Scl_fixture_004(request,L3ScaleEnhancement_Prologue_Epilogue):
    ...
    for i in range(0,data.num_of_vrfs):
        tg1.tg_emulation_bgp_control(handle=bgp_rtr1[i]['conf']['handle'], mode='stop')
        tg2.tg_emulation_bgp_control(handle=bgp_rtr2[i]['conf']['handle'], mode='stop')
        tg1.tg_emulation_bgp_control(handle=bgp_conf_3['handle'], mode='stop')
        tg2.tg_emulation_bgp_control(handle=bgp_conf_4['handle'], mode='stop')
        st.wait(5)
...
def L3Scl_fixture_005(request,L3ScaleEnhancement_Prologue_Epilogue):
    ...
    st.log("Mac count on {} is: {} & Mac count on {} is: {}".format(vars.D1,D5_mac_cnt,vars.D2,D6_mac_cnt))
...
def L3Scl_fixture_006(request,L3ScaleEnhancement_Prologue_Epilogue):
    ...
    (tg1, _, _, _, tg_ph_1, _, _, _) = get_handles()
    global h1,h2
    #tg1.tg_traffic_control(action='reset',port_handle=tg_ph_1)
    tg1.tg_interface_config(port_handle=tg_ph_1, handle=h1['handle'], mode='destroy')
    st.wait(5)
    tg1.tg_interface_config(port_handle=tg_ph_1, handle=h2['handle'], mode='destroy')
    st.wait(5)
...
def test_L3Scl_vrf_004(L3Scl_fixture_004):
    ...
    for i in range(0,data.num_of_vrfs):
        ...
        bgp_rtr1[i] = tgapi.tg_bgp_config(tg = tg1,
            handle    = h1[i]['handle'],
            conf_var  = conf_var,
            route_var = route_var,
            ctrl_var  = ctrl_start)
        ...
        # Configuring the BGP router.
        bgp_rtr2[i] = tgapi.tg_bgp_config(tg = tg2,
            handle    = h2[i]['handle'],
            conf_var  = conf_var2,
...
def create_l3_host(tg, tg_ph, host_count, src_ip, vlan, dest_ip, mac, dst_mac=None, cfg='yes'):
    ...
    if dst_mac != None:
        trf1b = tg.tg_traffic_config(mac_dst=dst_mac, rate_pps=str_rate, mode='create', port_handle=tg_ph,
...
def test_L3Scl_vrf_005(L3Scl_fixture_005):
    ...
    if not retry_api(verify_arp_count, vars.D1, expected_count=max_arp_count-5, vrf=data.vrf_name[0],  retry_count=6, delay=5):
        st.log('Expected ARP entries not found')
...
def test_L3Scl_vrf_006(L3Scl_fixture_006):
    ...
    h2 = tg1.tg_interface_config(port_handle=tg_ph_1, mode='config', ipv6_intf_addr=data.tg1_dut1_vrf_ipv6[1] ,  ipv6_gateway=data.dut1_tg1_vrf_ipv6[1], src_mac_addr='00:0b:01:00:00:01', vlan='1', vlan_id=data.dut1_tg1_vlan[1], count='8000', arp_send_req='1', ipv6_gateway_step='::', ipv6_intf_addr_step='::1', ipv6_prefix_length = '64')
    ############################################################################################
    hdrMsg("\n########## STEP 6.2 - Start ARP/ND from all hosts in TGEN ############\n")
    ############################################################################################
    tg1.tg_arp_control(handle=h1['handle'], arp_target='all')
    tg1.tg_arp_control(handle=h2['handle'], arp_target='all')
    ...
    snmp_max_arp_scale(tg1, h1, h2)
    st.report_pass('test_case_passed')

def snmp_max_arp_scale(tg, h1, h2):
    """
    Author : Prasad Darnasi<prasad.darnasi@broadcom.com>
    Verify The Q-BRIDGE-MIB Table requirements functionality by learning max hosts on DUT
    """
    global ipaddress
    ...
    st.banner('Doing clear ARP to restart ARP learning')
    ...
    snmp_output1 = snmp_obj.poll_for_snmp_walk(vars.D1, 15, 1, ipaddress=ipaddress,
                                  oid=data.oid_sysName, community_name=data.ro_community)
    basic_obj.get_top_info(vars.D1, proc_name='snmpd')
    snmp_output2 = snmp_obj.poll_for_snmp_walk(vars.D1, 15, 1, ipaddress=ipaddress, oid=data.oid_dot1qFdbTable,
    ...
    if snmp_output1 and snmp_output2:
        st.report_tc_pass("ft_snmp_scaling03", "test_case_passed")
    else:
        st.log('Generating the tech support for the subtest')
        basic_obj.get_techsupport(filename='snmp_max_arp_scale')
        st.report_tc_fail("ft_snmp_scaling03", "get_snmp_output_fail")
...
def test_L3Scl_ECMP_vrf_008(L3Scl_ECMP_fixture_008):
    ...
    ############################################################################################
    for _,vlan,ip1,ip2 in zip(range(0,data.max_ecmp),data.dut1_vlan_scl,data.dut1_ecmp_ipv6,data.dut2_ecmp_ipv6):
        dict1 = {'vrf_name':"Vrf1", 'intf_name':'Vlan'+vlan,'skip_error':True}
        dict2 = {'vrf_name':"Vrf1", 'intf_name':'Vlan'+vlan,'skip_error':True}
        parallel.exec_parallel(True, [vars.D1, vars.D2], vrf_api.bind_vrf_interface, [dict1, dict2])
