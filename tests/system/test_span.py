...
from utilities.common import poll_wait, random_vlan_list, iterable
...
def mirror_func_hooks(request):
    vlan_config()
    if st.get_func_name(request) in ['test_ft_mirroring_erspan_vlan']:
        if basic.get_hwsku(vars.D1).lower() in vars.constants[vars.D1]["TD3_PLATFORMS"]:
            vlanapi.create_vlan(vars.D1, mirror_data.vlan)
            vlanapi.add_vlan_member(vars.D1, mirror_data.vlan, [vars.D1T1P1, vars.D1T1P2])
    yield
    if st.get_func_name(request) in ['test_ft_mirroring_on_vlan_rx', 'test_ft_mirroring_erspan_span_check',
                                     'test_ft_mirroring_erspan']:
        mirror_acl_unconfig()
    elif st.get_func_name(request) in ['test_ft_mirroring_erspan_vlan']:
        if basic.get_hwsku(vars.D1).lower() in vars.constants[vars.D1]["TD3_PLATFORMS"]:
            vlanapi.delete_vlan_member(vars.D1, mirror_data.vlan, [vars.D1T1P1, vars.D1T1P2])
            vlanapi.delete_vlan(vars.D1, mirror_data.vlan)
        mirror_acl_unconfig()
...
def mirroring_erspan():
    ...
    if not poll_wait(mirror.verify_session_all, 10, vars.D1, mirror_type=mirror_data.mirror_type[1],
                                     session_name=mirror_data.session_name,status="active"):
...
def test_ft_mirroring_on_portchannel_rx():
    ...
    st.wait(5,"Waiting to create portchannel counters in DB")
    ensure_no_exception(exceptions)
    for output in out:
        if not output:
...
def test_ft_mirroring_gnmi():
    ...
    st.log(gnmi_set_out)
    rpc_error="rpc error:"
    if rpc_error in str(gnmi_set_out):
