...
def initialize_variables():
    ...
    pfc_data.vlan = str(random_vlan_list()[0])
    pfc_data.vlan_1 = str(random_vlan_list(exclude=[pfc_data.vlan])[0])
    pfc_data.vlan_2 = str(random_vlan_list(exclude=[pfc_data.vlan, pfc_data.vlan_1])[0])
...
def verify_loss_less_traffic(rx_traffic, tx_traffic):
    if int(rx_traffic) < int(tx_traffic):
        return False
    return True

def verify_lossy_traffic(rx_traffic, tx_traffic):
    if int(tx_traffic) <= int(rx_traffic):
        return False
    return True
...
def verify_pfc_counters(verify_pfc_counter_dict_list):
    for verify_pfc_counter_dict in verify_pfc_counter_dict_list:
        for pfc_priority in ['pfc0', 'pfc1', 'pfc2', 'pfc3', 'pfc4', 'pfc5', 'pfc6', 'pfc7']:
            if pfc_priority not in make_list(verify_pfc_counter_dict['counter']):
                if not filter_and_select(verify_pfc_counter_dict['pfc_counters_dict_list'], None, {pfc_priority: '0', 'port': verify_pfc_counter_dict['port'], 'port_mode': verify_pfc_counter_dict['mode']}):
                    st.error('PFC counter for port: {}, {} with mode: {} is incremented'.format(verify_pfc_counter_dict['port'], pfc_priority, verify_pfc_counter_dict['mode']))
                    return False
            else:
                ent = filter_and_select(verify_pfc_counter_dict['pfc_counters_dict_list'], [pfc_priority], {'port': verify_pfc_counter_dict['port'], 'port_mode': verify_pfc_counter_dict['mode']})
                if not (ent and int(ent[0][pfc_priority]) > 100):
                    st.error('PFC counter for port: {}, {} with mode: {} is not incremented'.format(verify_pfc_counter_dict['port'], pfc_priority, verify_pfc_counter_dict['mode']))
                    return False
    return True
    ...
    for port in ports_list:
        for mode in mode_list:
            for pfc_priority in ['pfc{}'.format(priority) for priority in range(8)]:
                if not filter_and_select(pfc_counters_dict_list, None, {'port_mode': mode, 'port': port, pfc_priority: '0'}):
                    st.error("The {} counter for port: {} at mode: {} is incremented".format(pfc_priority, port, mode))
                    return False
    return True
...
def verify_queue_counters(dut1_intf1, dut1_intf2, dut2_intf, queue_counters_dict, loss_less_list, multi_dut=False):
    ...
    try:
        if multi_dut:
            dut1_queue_cnt1 = int(filter_and_select(output[0], ['pkts_count'], {'txq': queue_counters_dict[vars.D1][dut1_intf1][0]})[0]['pkts_count'].replace(',', ''))
            dut1_queue_cnt2 = int(filter_and_select(output[0], ['pkts_count'], {'txq': queue_counters_dict[vars.D1][dut1_intf1][1]})[0]['pkts_count'].replace(',', ''))
        else:
            dut1_queue_cnt1 = int(filter_and_select(output[0], ['pkts_count'], {'txq': queue_counters_dict[vars.D1][dut1_intf1]})[0]['pkts_count'].replace(',', ''))
            dut1_queue_cnt2 = int(filter_and_select(interface_obj.show_queue_counters(vars.D1, dut1_intf2), ['pkts_count'], {'txq': queue_counters_dict[vars.D1][dut1_intf2]})[0]['pkts_count'].replace(',', ''))
        dut2_queue_cnt1 = int(filter_and_select(output[1], ['pkts_count'], {'txq': queue_counters_dict[vars.D2][dut2_intf][0]})[0]['pkts_count'].replace(',', ''))
        dut2_queue_cnt2 = int(filter_and_select(output[1], ['pkts_count'], {'txq': queue_counters_dict[vars.D2][dut2_intf][1]})[0]['pkts_count'].replace(',', ''))
...
def portchannel_config_epilog():
    ...
    if not delete_mac(vars.D1, pfc_data.destination_mac_dut1_tg2, pfc_data.vlan):
        st.report_fail("msg", "Failed to delete static MAC: {}".format(pfc_data.destination_mac_dut1_tg2))
...
def pfc_wd_verify(intf, queue, wd_stats_list, is_action_forward = True):
    temp_result_list = []
    if wd_stats_list is not False:
        for wd_stats in iterable(wd_stats_list):
            try:
        ...
        temp_result_list = [False, "PFC Watch dog statistics verification is failed"]
    return temp_result_list
...
def verify_pfc_counters_initialization(dut_list, port_list, thread = True):
    ...
    for index, dut in enumerate(dut_list, start = 0):
        params.append(ExecAllFunc(poll_wait, pfc_obj.verify_pfc_counters, 180, dut, port_list[index], mode = 'tx', pfc7 = 0))
...
def traffic_test_with_lossless_traffics(stream1, stream2, verify_clear_pfc_cnt= False):
    ...
    if not poll_wait(verify_queue_counters, 20, vars.D1D2P1, vars.D1D2P2, vars.D2T1P1, {vars.D1: {vars.D1D2P1: uc_list[0], vars.D1D2P2: uc_list[1]}, vars.D2: {vars.D2T1P1:uc_list}}, [True, True]):
...
def traffic_test_with_lossy_lossless_traffics(stream1, stream2, verify_clear_pfc_cnt= False):
    ...
    if not poll_wait(verify_queue_counters, 20, vars.D1D2P1, vars.D1D2P2, vars.D2T1P1, {vars.D1: {vars.D1D2P1: uc_list[0], vars.D1D2P2: uc_list[1]}, vars.D2: {vars.D2T1P1:uc_list}}, [True, False]):
        st.error("Queue counters verification failed")
        return False
...
def traffic_test_with_lossyrx_lossless_traffics(stream1, stream2, is_asym=False, verify_clear_pfc_cnt= False):
    ...
    if not poll_wait(verify_queue_counters, 20, vars.D1D2P1, vars.D1D2P2, vars.D2T1P1, {vars.D1: {vars.D1D2P1: uc_list[0], vars.D1D2P2: uc_list[1]}, vars.D2: {vars.D2T1P1:uc_list}}, [is_asym, True]):
        st.error("Queue counters verification failed")
        return False
...
def traffic_test_congestion_in_both_duts(stream1, stream2, stream3, is_asym=False):
    ...
    st.log("Verifying queue counters")
    uc_list = ['UC{}'.format(pfc_data.asym_lossless_priority), 'UC{}'.format(pfc_data.lossless_priorities[1])]
    if not poll_wait(verify_queue_counters, 12, vars.D1D2P1, None, vars.D2T1P1, {vars.D1: {vars.D1D2P1: uc_list}, vars.D2: {vars.D2T1P1:uc_list}}, [is_asym, True], multi_dut=True):
        st.error("Queue counters verification failed")
        return False
    st.log('#####################Traffic test with both lossyrx, loss-less traffics is passed#####################')
    return True
...
def traffic_test_with_pfc_disable(stream1, stream2):
    ...
    st.log("Verifying queue counters")
    uc_list = ['UC{}'.format(pfc_data.lossless_priorities[0]), 'UC{}'.format(pfc_data.all_lossy_priority)]
    if not poll_wait(verify_queue_counters, 20, vars.D1D2P1, vars.D1D2P2, vars.D2T1P1, {vars.D1: {vars.D1D2P1: uc_list[0], vars.D1D2P2: uc_list[1]}, vars.D2: {vars.D2T1P1:uc_list}}, [False, False]):
        st.error("Queue counters verification failed")
        return False
...
def traffic_test_for_pfcwd(dut, stream_list, detection_time_list, interface_list, queue_list, is_action_forward_list):
    ...
    wait_time = int((int(detection_time_list[0])+int(detection_time_list[1]))/100) + 4
    st.wait(wait_time)
...
def test_ft_pfc_with_shut_noshut():
    ...
    for _ in range(5):
        ...
        ensure_no_exception(exceptions)
    [output, exceptions] = exec_all(True, [ExecAllFunc(poll_wait, interface_obj.verify_interface_status, 60, vars.D1, [vars.D1T1P1, vars.D1T1P2, vars.D1D2P1, vars.D1D2P2], 'oper', 'up'), ExecAllFunc(poll_wait, interface_obj.verify_interface_status, 60, vars.D2, [vars.D2T1P1, vars.D2T1P2, vars.D2D1P1, vars.D2D1P2], 'oper', 'up')])
    ensure_no_exception(exceptions)
    if not all(output):
